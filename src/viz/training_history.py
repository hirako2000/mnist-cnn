...
def plot_comparison(history_relu, history_tanh, output_dir: Path):
    """
...
def main():
    ...
    # Handle comparison
        ...
        if has_tanh and has_relu:
            print("\n📊 Generating ReLU vs Tanh comparison...")
            history_tanh = load_history(tanh_path)
            history_relu = load_history(relu_path)
            plot_comparison(history_relu, history_tanh, output_dir)
        else:
            print("❌ Need both Tanh and ReLU history files for comparison")
